...
int iot_mqtt_init(void)
{
    /* 创建独立线程等待网络就绪并启动 MQTT，避免各种回调被误注销 */
    rt_thread_t tid = rt_thread_create("mqtt_app", mqtt_app_entry, RT_NULL, 2048, 22, 10);
    if (tid != RT_NULL)
    {
